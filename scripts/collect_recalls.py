"""
国交省リコール届出を一括収集し、raw.json を生成するスクリプト。

処理フロー:
  1. recall.html から月別プレスリリースリンクを収集
  2. 各プレスリリースページから PDF URL を取得
  3. PDF を OCR → raw.json を生成
  4. dataset/recalls/{recall_id}/ に保存

fta-evaluator など下流のリポジトリはこの raw.json を「すでにあるもの」として
読み込むだけで、収集そのものはこのリポジトリ（fta-spec-generator）に閉じる。

実行:
  python scripts/collect_recalls.py --year 2026 --months 1-6   # 2026年1〜6月
  python scripts/collect_recalls.py --year 2026 --months 1     # 1月のみ
  python scripts/collect_recalls.py --year 2026 --months 1-6 --dry-run  # URLリストのみ表示
"""
...
from pipeline import fetch_pdf, ocr_pdf, extract_recall_metadata, build_aoai_client

RECALL_ROOT = Path(__file__).parent.parent
DATA_DIR = RECALL_ROOT / "data"
...
def _collect_press_links() -> list[RecallPressEntry]:
    """recall.html からプレスリリースページの URL を収集する。"""
...
def _enrich_with_pdfs(
    ...
    """各プレスリリースページから PDF URL・届出日を取得し、年月フィルタを適用する。"""
...
def _process_entry(entry: RecallPressEntry, aoai_client) -> list[str]:
    """1件のプレスリリースを収集・OCR・構造化して dataset/recalls/{recall_id}/ に保存する。
...
def collect(year: int, months: list[int], dry_run: bool = False) -> list[str]:
    """指定年月のリコール届出を収集する。
    ...
    Returns:
        収集した recall_id のリスト
    """
    press_entries = _collect_press_links()
    enriched = _enrich_with_pdfs(press_entries, year, months)
...
def _parse_months(spec: str) -> list[int]:
    """"1-6" や "1,3,5" 形式の月指定を月番号のリストに変換する。

    Args:
        spec: 月指定文字列（例: "1-6", "1,3,5", "1"）
    ...
    for part in spec.split(","):
        ...
        if "-" in part:
            lo, hi = part.split("-")
            months.update(range(int(lo), int(hi) + 1))
        elif part:
            months.add(int(part))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="国交省リコール届出を一括収集し raw.json を生成")
    parser.add_argument("--year", type=int, required=True, help="収集対象年（西暦、例: 2026）")
    parser.add_argument("--months", type=str, required=True, help="収集対象月（例: 1-6, 1,3,5）")
    parser.add_argument("--dry-run", action="store_true",
                        help="URL 一覧のみ表示（ダウンロード・OCR しない）")
    args = parser.parse_args()

    months = _parse_months(args.months)
    collect(args.year, months, dry_run=args.dry_run)
