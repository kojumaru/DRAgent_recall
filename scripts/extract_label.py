...

{
  ...
  "failure_modes": [
    "故障モード（浸水・短絡・腐食・疲労破壊・締結不良など物理的な故障現象）"
  ],
  "top_event": [
    "FTAのトップ事象となる最悪の結果（例：火災・走行不能・意図しない動作）"
  ],
  "causal_chain": [
    "根本原因から始まりトップ事象で終わる故障の因果連鎖（順番通りに列挙）"
  ]
}

## 抽出ルール
- target_component: リコール届出書に記載された部品名（別名・略称も含む）。2〜5件
- failure_modes: 物理的・化学的な故障現象のみ。「回路設計不十分」等の原因は除く。1〜4件
- top_event: 「最悪の場合〜おそれ」に相当する事象。1〜2件
- causal_chain: 根本原因 → 中間事象 → ... → トップ事象 の順。3〜8件
- 記載がない項目は空配列 [] とする
"""
