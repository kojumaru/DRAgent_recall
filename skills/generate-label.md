...
- **「〜機能を喪失する」「〜機能が失われる」「〜機能が低下する」は使わない。** 代わりに具体的な物理変化動詞を使う:
  - 機械部品: 折損する / 破断する / 亀裂が発生する / 変形する / 摩耗する / 緩む / 脱落する
  - 電気・電子: 短絡する / 断線する / 腐食する / 誤検知する / 誤演算する
  - 流体系: 漏れる / 詰まる / 破損する
  - 素材系: 剥離する / 劣化する / 損傷する / 膨潤する
- **初期状態の記述を故障モードとして使わない。** 「〜されていない」「〜が不足している」「〜が誤っている」等の状態表現はNG。故障モードは必ず変化・事象を表す動詞で終わること
- **カスケード事象（故障の結果として起きる後続事象）を故障モードに含めない。** 故障モードは対象部品自体に起きる直接的な物理変化のみ。後続事象（脱落・接触・発火等）は causal_chain に記載する
- 故障モードが2件以上の場合、互いに「原因→結果」の関係になっていないか確認する（なっている場合はカスケード事象の混入）
...
**状況説明を含めない:** 「走行中に」「エンジン始動時に」「使用を続けると」等の発生状況はトップ事象に含めない。結果のみを記述する（例: 「走行中に走行不能となる」→「走行不能となる」）
