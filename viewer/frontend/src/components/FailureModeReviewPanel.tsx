...
}: { current: ItemVerdict | null; value: ItemVerdict; onClick: () => void }) {
...
export default function FailureModeReviewPanel({
  recallId,
  failureModes,
  existingReview,
  onSaved,
  ...
  recallId: string;
  failureModes: string[];
  existingReview: FailureModeReview | null;
  onSaved: (r: FailureModeReview) => void;
  apiBase?: string;
}) {
  const [reviewer, setReviewer] = useState(existingReview?.reviewer ?? '');
  const [itemReviews, setItemReviews] = useState<Record<string, ItemVerdict | null>>(() => {
    const init: Record<string, ItemVerdict | null> = {};
    for (const m of failureModes) {
      init[m] = (existingReview?.item_reviews?.[m] as ItemVerdict) ?? null;
    }
    return init;
  });
  ...
  const allDone = failureModes.length > 0 && reviewedCount === failureModes.length;
  ...
  const approveAll = () => {
    setItemReviews((prev) => {
      const next = { ...prev };
      for (const m of failureModes) next[m] = 'approved';
      return next;
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const body: FailureModeReviewSubmission = {
        reviewer,
      ...
      const res = await fetch(`${apiBase}/${recallId}/failure_mode_review`, {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 text-[13px]">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-neutral-800">故障モードレビュー</h2>
        <span className="text-[11px] text-neutral-400">{reviewedCount}/{failureModes.length} 評価済み</span>
      </div>

      <p className="text-[11px] leading-relaxed text-neutral-500">
        LLMが抽出した各故障モードが、リコール届出書の内容として正しいかを評価してください。
        欠落している故障モードがあれば「不足項目を追加」で記録します。
      </p>
      ...
      <div className="flex flex-col gap-2">
        {failureModes.length === 0 && (
          <p className="text-[12px] text-neutral-400">故障モードがありません。</p>
        )}
        {failureModes.map((mode) => {
          const verdict = itemReviews[mode] ?? null;
                ...
                <div className="flex shrink-0 gap-1">
                  {(['approved', 'needs_fix'] as ItemVerdict[]).map((v) => (
                    <VerdictButton
                      ...
                      current={verdict}
                      value={v}
                      onClick={() => setItemReviews((prev) => ({ ...prev, [mode]: v }))}
                    />
                  ))}
                </div>
              </div>
      ...
      <div className="rounded border border-amber-200 bg-amber-50 p-3">
        <p className="mb-2 text-[11px] font-semibold text-amber-800">不足している故障モードを追加（任意）</p>
      ...
      <div className="flex flex-col gap-2 border-t border-neutral-200 pt-3">
        ...
        >
          {saving ? '保存中...' : allDone ? 'レビューを保存' : `残り ${failureModes.length - reviewedCount} 件`}
        </button>
      </div>
